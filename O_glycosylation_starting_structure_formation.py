...
class OGlycosylationStartingStructureFormation:
    ...
    def _prepare_acceptor_peptide_with_glycan(self):
        ...
        self.acceptor_peptide_pose = pyrosetta.pose_from_sequence(
            self.acceptor_peptide_sequence
        )
        if self.acceptor_peptide_sugar_name != None:
            glycosylator = SimpleGlycosylateMover()
        if self.acceptor_peptide_sugar_name not in sugar_dictionary:
            print("The sugar not known!!")
        glycosylator.set_glycosylation(
            sugar_dictionary[self.acceptor_peptide_sugar_name]
        )
    ...
    def _merge_two_pose_by_jump(self, enzyme_pose, acceptor_peptide_pose):
        ...
        anchor_point = (
            int(enzyme_pose.total_residue()) - 2
        )  # just to make sure that the UDP-sugar remains the last residues in the whole pose
        enzyme_pose.append_pose_by_jump(acceptor_peptide_pose, anchor_point)

    def _make_fold_tree_docking_friendly(self, enzyme_pose):
        """
        The function changes the fold_tree of the pose from the default one to a 
        more dockign friendly. 
        """
        chain_begin_list = []
        chain_end_list = []
        ...
        # Criteria 2 and 3: the acceptor peptide will not have any sugar 
        # in it and the donor molecule will have UDP it it
        for i in range(0, len(chain_length_list)):
        ...
        reference = enzyme_pose.residue(
            chain_begin_list[self.donor_chain - 1]
        ).xyz("3OPB")
        distance_lowest = float("inf")
        pivot_residue = float("inf")
        for i in range(
            chain_begin_list[self.enzyme_chain - 1],
            chain_end_list[self.enzyme_chain - 1] + 1,
        ):
        ...
        first_residue_substrate = int(
            enzyme_pose.chain_begin(self.acceptor_chain)
        )
        last_residue_substrate = int(
            enzyme_pose.chain_end(self.acceptor_chain)
        )
        anchor_residue_substrate = int(
            (first_residue_substrate + last_residue_substrate) / 2
        )
        ...
        ft_docking.add_edge(
            anchor_residue_substrate, first_residue_substrate, -1
        )  # downstream base peptide
        ft_docking.add_edge(
            anchor_residue_substrate, last_residue_substrate, -1
        )  # upstream base peptide
        ft_docking.add_edge(
            first_donor, last_donor, -1
        )  # the donor UDP-GlcNAc backbone

        chemical_edges = (
            enzyme_pose.fold_tree().get_chemical_edges()
        )  # connections to the glycan residues from anchor out
        for chemical in range(1, len(chemical_edges) + 1):
            ft_docking.add_edge(
                chemical_edges[chemical].start(),
                chemical_edges[chemical].stop(),
                -2,
            )
            ...
            ft_docking.add_edge(
                self.sugar_chain_start, self.sugar_chain_end, -1
            )
    ...
    def _relax_only_acceptor_peptide(self, curr_enzyme_pose):
        ...
        for j in range(
            int(curr_enzyme_pose.chain_begin(self.acceptor_chain)),
            int(curr_enzyme_pose.total_residue()) + 1,
        ):  # Can be generalised
        ...
        prevent_subset_repacking = OperateOnResidueSubset(
            prevent_repackign_rlt, acceptor_peptide_selector, flip_subset=True
        )
        ...
        mmf.all_chi(True)
        mmf.add_bb_action(
            pyrosetta.rosetta.core.select.movemap.move_map_action.mm_enable,
            acceptor_peptide_selector,
        )
    ...
    def _relax_the_interface(self, curr_enzyme_pose):
        ...
        for j in range(
            ...
            acceptor_peptide_selector.append_index(j)
        acceptor_peptide_selector.apply(curr_enzyme_pose)
        acceptor_peptide_neighborhood_selector.set_focus_selector(
            acceptor_peptide_selector
        )
        acceptor_peptide_neighborhood_selector.set_include_focus_in_subset(
            True
        )

        prevent_repackign_rlt = PreventRepackingRLT()
        prevent_subset_repacking = OperateOnResidueSubset(
            prevent_repackign_rlt,
            acceptor_peptide_neighborhood_selector,
            flip_subset=True,
        )
    ...
    def _add_acceptor_peptide_and_enzyme(self, decoy_numbers):
        """ This program takes the base sugar pose and the pose of the enzyme with 
        donor moiety and the constraints and make sure the they obey the constraint
        in subsequent relax procedure as a procedure to bypass manual overlaying 
        using PyMOL. """
        ...
        self._merge_two_pose_by_jump(
            self.input_enzyme_pose, self.acceptor_peptide_pose
        )
        ...
        if self.reference_pose_file:
            self.reference_pose = pyrosetta.pose_from_file(
                self.reference_pose_file
            )
        ...
        for trial_number in range(0, decoy_numbers):
            ...
            if self.reference_pose_file:
                ...
                atom1 = curr_enzyme_pose.residue(
                    int(curr_enzyme_pose.chain_end(self.donor_chain))
                ).xyz("C1")
                atom2 = curr_enzyme_pose.residue(
                    int(curr_enzyme_pose.chain_end(self.acceptor_chain)) + 1
                ).xyz("O6")
                ...
                for j in range(
                    int(curr_enzyme_pose.chain_begin(self.enzyme_chain)),
                    int(curr_enzyme_pose.chain_end(self.enzyme_chain)) + 1,
                ):
                ...
                core1_rmsd.set_residue_selector_super_reference(
                    allEnzyme_reference
                )
                core1_rmsd.set_rmsd_type(
                    pyrosetta.rosetta.core.scoring.rmsd_all_heavy
                )
                core1_rmsd_list.append(
                    float(core1_rmsd.calculate(curr_enzyme_pose))
                )
                ## -----------------------------------------------------------
                ...
                peptide_rmsd.set_residue_selector_super_reference(
                    allEnzyme_reference
                )
                peptide_rmsd.set_rmsd_type(
                    pyrosetta.rosetta.core.scoring.rmsd_all
                )
                peptide_rmsd_list.append(
                    float(peptide_rmsd.calculate(curr_enzyme_pose))
                )
                ## -----------------------------------------------------------

        ## Printing out the benchmarking inforfmation and storing it inside a 
    ...
    def apply(self, input_pose):
        """
        This is the function the user should use to apply the full "Mover" on 
        the input_pose which is actyally the enzyme + donor pose and return 
        the pose with enzyme + donor residue + acceptor residue.
        """
...
def argument_parsing():
    info = """
        This function let's the user input all the command line arguments for 
    ...
    parser.add_argument(
        ...
        required=False,
        default=False,
        help="Input if you want to repack the interface region \
    ...
    parser.add_argument(
        ...
        required=False,
        default=1,
        help="Input the number of independent decoys the user wants.",
    )
    parser.add_argument(
        "-out_pdb",
        "--output_pdb",
        ...
        required=False,
        default=False,
        help="Enter True, if you want the pdb of the output file.",
    )
    parser.add_argument(
        "-out_dir",
        "--ouput_pdb_directory",
        ...
        required=False,
        default="",
        help="Input the direcotory where you want your output_pdb file \
        to be saved.",
    )
    parser.add_argument(
        "-out_name",
...
if __name__ == "__main__":
    ...
    init_flags = (
        "-include_sugars -maintain_links -auto_detect_glycan_connections -alternate_3_letter_codes pdb_sugar"
        + " -constraints:cst_fa_file "
        + mover.constraints_file
    )
